...
router.get("/agenda.ics", async (req, res) => {
  const professor = await resolverProfessorLogado(req);
  if (!professor) {
    res.status(404).send("Nenhum professor vinculado a esta conta.");
    return;
  }
  const escolaId = getEscolaId(req);
  ...
  const diaSemanaHoje = agora.getDay() === 0 ? 6 : agora.getDay() - 1;
  const segundaRef = new Date(agora);
  segundaRef.setDate(agora.getDate() - diaSemanaHoje);
  ...
  aulas.forEach((aula, idx) => {
    ...
    linhas.push(
      "BEGIN:VEVENT",
      `UID:aula-${idx}-${professor.id}@nexgrade`,
  ...
  reservasAtivas.forEach((r, idx) => {
    const yyyymmdd = r.data.replace(/-/g, "");
    const hora = HORAS_POR_AULA[r.numeroAula] ?? "0800";
    linhas.push(
      "BEGIN:VEVENT",
      `UID:reserva-${idx}-${professor.id}@nexgrade`,
      `DTSTART:${yyyymmdd}T${hora}00`,
      `DTEND:${yyyymmdd}T${hora}50`,
      `SUMMARY:[${r.status === "confirmada" ? "Confirmada" : "Pendente"}] ${r.titulo}`,
      `LOCATION:${r.salaNome}`,
      "END:VEVENT",
    );
  });
  ...
  res.setHeader("Content-Disposition", 'attachment; filename="minha-agenda.ics"');
  res.send(ics);
});
