...
// [NOVO] RF-TUR-XX: troca de horário via arrastar-e-soltar na grade da
// turma (artifacts/horario-escolar/src/pages/turmas/horario.tsx).
//
// Sempre valida ANTES de gravar: verifica se o(s) professor(es)
// envolvido(s) ficariam com um conflito real de agenda no novo
// horário -- já dando aula em OUTRA turma no mesmo dia/aula, ou
// marcado como indisponível (ver disponibilidadeTable). Isso cobre
// justamente o tipo de conflito que a grade de uma única turma não
// consegue mostrar sozinha (o professor pode estar livre nessa turma
// mas ocupado em outra). Se achar conflito, não grava nada e devolve
// 409 com as mensagens -- quem chamou decide se quer confirmar mesmo
// assim com `forcar: true`.
//
// Suporta dois casos:
//  1. Slot de destino VAZIO -> move a aula de origem pra lá (sem
//     parceiro de troca).
//  2. Slot de destino OCUPADO (por um único professor, sem
//     co-docência) -> troca as duas aulas de lugar.
//
// Co-docência (mais de um professor no mesmo dia/aula, mesma turma)
// não é suportada nessa troca ainda -- rejeita com uma mensagem clara
// em vez de tentar adivinhar qual das duas linhas mover.
const TrocarHorarioBody = z.object({
...
router.post("/trocar", async (req, res) => {
  const escolaId = getEscolaId(req);
  const parsed = TrocarHorarioBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const { origemId, diaSemana, numeroAula, forcar } = parsed.data;

  const origem = await db.select().from(horariosTable)
    .where(and(eq(horariosTable.id, origemId), eq(horariosTable.escolaId, escolaId)))
    .then(r => r[0]);
  if (!origem) {
    res.status(404).json({ error: "Aula de origem não encontrada" });
    return;
  }

  if (origem.diaSemana === diaSemana && origem.numeroAula === numeroAula) {
    res.status(400).json({ error: "Selecione um horário diferente do atual" });
    return;
  }

  const destinoRows = await db.select().from(horariosTable)
    .where(and(
      eq(horariosTable.escolaId, escolaId),
      eq(horariosTable.turmaId, origem.turmaId),
      eq(horariosTable.diaSemana, diaSemana),
      eq(horariosTable.numeroAula, numeroAula),
    ));

  if (destinoRows.length > 1) {
    res.status(400).json({
      error: "Esse horário tem co-docência (mais de um professor) — troca por arrastar não é suportada aqui ainda. Ajuste pelo cadastro ou regenere o horário.",
    });
    return;
  }
  const destino = destinoRows[0] ?? null;

  const conflitos: string[] = [];

  const professorOrigemOcupadoAltrove = (
    await db.select().from(horariosTable)
      .where(and(
        eq(horariosTable.escolaId, escolaId),
        eq(horariosTable.professorId, origem.professorId),
        eq(horariosTable.diaSemana, diaSemana),
        eq(horariosTable.numeroAula, numeroAula),
      ))
  ).filter(r => r.id !== origem.id && r.id !== destino?.id);
  if (professorOrigemOcupadoAltrove.length > 0) {
    conflitos.push("O professor dessa aula já dá aula em outra turma nesse novo horário.");
  }

  const indispOrigem = await db.select().from(disponibilidadeTable)
    .where(and(
      eq(disponibilidadeTable.professorId, origem.professorId),
    ...
    .then(r => r[0]);
  if (indispOrigem) {
    conflitos.push(`O professor dessa aula está marcado como indisponível nesse novo horário${indispOrigem.motivo ? ` (${indispOrigem.motivo})` : ""}.`);
  }

  if (destino) {
    const professorDestinoOcupadoAltrove = (
      await db.select().from(horariosTable)
        .where(and(
          eq(horariosTable.escolaId, escolaId),
          eq(horariosTable.professorId, destino.professorId),
          eq(horariosTable.diaSemana, origem.diaSemana),
          eq(horariosTable.numeroAula, origem.numeroAula),
        ))
    ).filter(r => r.id !== origem.id && r.id !== destino.id);
    if (professorDestinoOcupadoAltrove.length > 0) {
      conflitos.push("O professor da outra aula já dá aula em outra turma no horário original.");
    }

    const indispDestino = await db.select().from(disponibilidadeTable)
      .where(and(
        eq(disponibilidadeTable.professorId, destino.professorId),
        eq(disponibilidadeTable.diaSemana, origem.diaSemana),
        eq(disponibilidadeTable.horarioSlot, origem.numeroAula),
        eq(disponibilidadeTable.disponivel, false),
      ))
      .then(r => r[0]);
    if (indispDestino) {
      conflitos.push(`O professor da outra aula está marcado como indisponível no horário original${indispDestino.motivo ? ` (${indispDestino.motivo})` : ""}.`);
    }
  }

  if (conflitos.length > 0 && !forcar) {
    res.status(409).json({ error: "Conflito detectado", conflitos });
    return;
  }

  // [IMPORTANTE] Troca via DELETE + INSERT (não UPDATE) das duas
  // linhas dentro de uma transação -- evita qualquer problema de
  // constraint (UNIQUE ou CHECK) que uma atualização intermediária
  // pudesse disparar se, por exemplo, tentássemos mover a origem pra
  // um valor temporário antes de mover o destino pro lugar dela.
  const turmaId = origem.turmaId;
  await db.transaction(async (tx) => {
    if (destino) {
      await tx.delete(horariosTable).where(eq(horariosTable.id, origem.id));
      await tx.delete(horariosTable).where(eq(horariosTable.id, destino.id));
      await tx.insert(horariosTable).values([
        {
          escolaId, turmaId: origem.turmaId, disciplinaId: origem.disciplinaId,
          professorId: origem.professorId, diaSemana, numeroAula, sala: origem.sala,
        },
        {
          escolaId, turmaId: destino.turmaId, disciplinaId: destino.disciplinaId,
          professorId: destino.professorId, diaSemana: origem.diaSemana, numeroAula: origem.numeroAula, sala: destino.sala,
        },
      ]);
    } else {
      await tx.update(horariosTable)
        .set({ diaSemana, numeroAula })
        .where(eq(horariosTable.id, origem.id));
    }
  });

  const atualizados = await db.select().from(horariosTable).where(eq(horariosTable.turmaId, turmaId));
  const enriched = await Promise.all(atualizados.map(enrichSlot));
  res.json({ ok: true, conflitos, horario: enriched });
});
