...
export function detectType(headers: string[]): "professores" | "turmas" | "disciplinas" | "desconhecido" {
  ...
  if (h.some(x => x.includes("turma") || x.includes("serie") || x.includes("série"))) return "turmas";
  if (h.some(x => x.includes("disciplina") || x.includes("materia") || x.includes("matéria"))) return "disciplinas";
  return "desconhecido";
}
...
router.post("/preview", async (req, res) => {
  ...
  if (rows.length < 2) { res.status(400).json({ error: "CSV deve ter cabeçalho + dados" }); return; }
...
router.post("/confirmar", async (req, res) => {
  ...
  if (tipo === "professores") {
    for (const [li, row] of dados.entries()) {
      ...
      if (!nome || !email) { erros.push(`Linha ${li + 2}: nome e email obrigatórios`); continue; }
      try {
    ...
    for (const [li, row] of dados.entries()) {
      ...
      const serie = row[idx(["serie", "série", "ano"])]?.trim();
      if (!nome) { erros.push(`Linha ${li + 2}: nome obrigatório`); continue; }
      try {
