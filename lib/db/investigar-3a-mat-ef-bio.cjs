...
async function main() {
  ...
  const { rows } = await client.query(
    `SELECT td.id AS vinculo_id, d.nome AS disciplina, d.codigo_sae, p.nome AS professor,
     ...
     WHERE td.turma_id = $1 AND (d.nome ILIKE '%matemática%' OR d.nome ILIKE '%financeira%' OR d.nome ILIKE '%biologia%')
     ORDER BY d.nome`,
    [turma[0].id, turma[0].matriz_curricular_id]
  );
  for (const r of rows) {
    console.log(`  vinculo_id=${r.vinculo_id} | "${r.disciplina}" (sae=${r.codigo_sae}) | professor="${r.professor}" | matriz=${r.matriz} | override=${r.override}`);
  }
