...
async function main() {
  ...
  try {
    ...
    console.log(`Total de disciplinas do Mário Braga: ${todasRes.rows.length}\n`);
    ...
    fs.writeFileSync(
      path.join(__dirname, 'pente-fino-mario-braga.json'),
      JSON.stringify({ dupNome, truncados, conflitosNumeral }, null, 2)
    );
    console.log('\n\nSalvo em: pente-fino-mario-braga.json');
  } finally {
    await client.end();
  }
}
