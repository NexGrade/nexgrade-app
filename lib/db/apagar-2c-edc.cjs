...
async function main() {
  ...
  if (rows.length !== 1) { console.error('ERRO: vinculo não encontrado.'); await client.end(); process.exit(1); }
  console.log(`  ${rows[0].turma} / "${rows[0].disciplina}" (id=${VINCULO}) -> APAGAR`);
  ...
  try {
    await client.query(`DELETE FROM turma_disciplinas WHERE id = $1`, [VINCULO]);
