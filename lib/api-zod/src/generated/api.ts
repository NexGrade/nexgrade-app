...
export const GetProfessorCargaResponse = zod.object({
  ...
  "porTurno": zod.record(zod.string(), zod.number()).optional().describe('Total de aulas semanais do professor, agrupado por turno (matutino\/vespertino\/noturno).'),
  "haInstitucionalPorTurno": zod.record(zod.string(), zod.number()).optional().describe('HA institucional necessaria por turno, conforme regra SEED 15\/5 (RNF-SEED-01).'),
  "haInstitucionalTotal": zod.number().optional().describe('Soma da HA institucional necessaria em todos os turnos.'),
  "haAlocadaPorTurno": zod.record(zod.string(), zod.number()).optional().describe('Quantas HA obrigatorias ja estao de fato marcadas em disponibilidade_professores, por turno.')
})
...
export const ListDisciplinasResponseItem = zod.object({
  ...
  "cor": zod.string(),
  "codigoSae": zod.string().nullish().describe('CÃƒÂ³digo SAE oficial (SEED-PR), quando conhecido.'),
...
export const ListTurmasResponseItem = zod.object({
  ...
  "maxAulasConsecutivasDia": zod.number().nullish().describe('Limite de aulas geminadas desta disciplina, nesta turma, por dia. Nulo = usa o padrÃƒÂ£o geral (configuracoes\/seed_pr.max_aulas_geminadas_padrao).'),
  "grupoCompartilhadoId": zod.string().nullish().describe('Quando preenchido, agrupa esta disciplina com a mesma linha de outras turmas para horÃƒÂ¡rio simultÃƒÂ¢neo (ex. ItinerÃƒÂ¡rio Formativo cursado junto).')
})).optional().describe('Carga horÃƒÂ¡ria semanal efetiva por disciplina (override da matriz aplicada, com fallback para a carga global).'),
  "createdAt": zod.string()
})
...
})).optional().describe('ConfiguraÃƒÂ§ÃƒÂ£o extra por disciplina Ã¢â‚¬â€ chave ÃƒÂ© o disciplinaId (como string). SÃƒÂ³ tem efeito para disciplinas presentes em disciplinaIds.')
})
...
export const UpdateTurmaBody = zod.object({
  ...
  "maxAulasConsecutivasDia": zod.number().min(1).optional(),
  "grupoCompartilhadoId": zod.string().optional()
})).optional().describe('ConfiguraÃƒÂ§ÃƒÂ£o extra por disciplina Ã¢â‚¬â€ chave ÃƒÂ© o disciplinaId (como string). Pode ser enviado mesmo sem disciplinaIds, para sÃƒÂ³ atualizar a configuraÃƒÂ§ÃƒÂ£o de vÃƒÂ­nculos jÃƒÂ¡ existentes.')
})
...
export const ListDisponibilidadeResponseItem = zod.object({
  ...
  "turno": zod.union([zod.literal('matutino'),zod.literal('vespertino'),zod.literal('noturno'),zod.literal(null)]).nullish().describe('Turno deste bloqueio Ã¢â‚¬â€ necessÃƒÂ¡rio para validar concentraÃƒÂ§ÃƒÂ£o de hora-atividade no mesmo turno das aulas (ResoluÃƒÂ§ÃƒÂ£o SEED n.Ã‚Âº 7.200\/2025, art. 11, Ã‚Â§4Ã‚Âº).'),
  "horaAtividadeObrigatoria": zod.boolean().default(listDisponibilidadeResponseHoraAtividadeObrigatoriaDefault).describe('true quando este bloqueio representa Hora-Atividade obrigatÃƒÂ³ria, nÃƒÂ£o outro tipo de indisponibilidade.')
})
...
export const SetDisponibilidadeResponse = zod.object({
  ...
  "motivo": zod.string().nullish(),
  "turno": zod.union([zod.literal('matutino'),zod.literal('vespertino'),zod.literal('noturno'),zod.literal(null)]).nullish().describe('Turno deste bloqueio Ã¢â‚¬â€ necessÃƒÂ¡rio para validar concentraÃƒÂ§ÃƒÂ£o de hora-atividade no mesmo turno das aulas (ResoluÃƒÂ§ÃƒÂ£o SEED n.Ã‚Âº 7.200\/2025, art. 11, Ã‚Â§4Ã‚Âº).'),
  "horaAtividadeObrigatoria": zod.boolean().default(setDisponibilidadeResponseHoraAtividadeObrigatoriaDefault).describe('true quando este bloqueio representa Hora-Atividade obrigatÃƒÂ³ria, nÃƒÂ£o outro tipo de indisponibilidade.')
})
...
export const SetDisponibilidadeLoteResponseItem = zod.object({
  ...
  "motivo": zod.string().nullish(),
  "turno": zod.union([zod.literal('matutino'),zod.literal('vespertino'),zod.literal('noturno'),zod.literal(null)]).nullish().describe('Turno deste bloqueio Ã¢â‚¬â€ necessÃƒÂ¡rio para validar concentraÃƒÂ§ÃƒÂ£o de hora-atividade no mesmo turno das aulas (ResoluÃƒÂ§ÃƒÂ£o SEED n.Ã‚Âº 7.200\/2025, art. 11, Ã‚Â§4Ã‚Âº).'),
  "horaAtividadeObrigatoria": zod.boolean().default(setDisponibilidadeLoteResponseHoraAtividadeObrigatoriaDefault).describe('true quando este bloqueio representa Hora-Atividade obrigatÃƒÂ³ria, nÃƒÂ£o outro tipo de indisponibilidade.')
})
...
export const ListLimitesDiariosProfessorResponseItem = zod.object({
  "id": zod.number(),
  "professorId": zod.number(),
  "turmaId": zod.number().nullish().describe('Nulo = padrÃƒÂ£o do professor, vÃƒÂ¡lido em qualquer turma. Preenchido = override especÃƒÂ­fico por turma.'),
...
export const SetLimiteDiarioProfessorBody = zod.object({
  "professorId": zod.number(),
  "turmaId": zod.number().nullish().default(setLimiteDiarioProfessorBodyTurmaIdDefault).describe('Nulo = padrÃƒÂ£o do professor, vÃƒÂ¡lido em qualquer turma. Preenchido = override especÃƒÂ­fico por turma.'),
  "maxAulasPorDia": zod.number().min(1)
})
...
export const ListPlanosResponseItem = zod.object({
  "id": zod.number(),
  "nome": zod.string(),
  "precoMensal": zod.number().describe('PreÃ§o mensal em centavos. 0 = plano gratuito (Piloto).'),
  "precoAnual": zod.number().nullish().describe('PreÃ§o anual em centavos, com desconto. Nulo enquanto nÃ£o configurado.'),
...
export const GetEscolaAtualResponse = zod.object({
  "cadastrada": zod.boolean().describe('false quando o usuÃƒÂ¡rio\/organizaÃƒÂ§ÃƒÂ£o ainda nÃƒÂ£o passou pelo onboarding.'),
...
export const CadastrarEscolaBody = zod.object({
  ...
  "turnosOfertados": zod.string().optional().describe('Lista separada por vÃ­rgula, ex: \'matutino,vespertino\'.'),
...
export const CadastrarEscolaResponse = zod.object({
  ...
  "codigoInep": zod.string().nullish().describe('CÃ³digo INEP oficial da escola perante o MEC.'),
  "nre": zod.string().nullish().describe('NÃºcleo Regional de EducaÃ§Ã£o (SEED-PR) ou equivalente estadual.'),
  "turnosOfertados": zod.string().nullish().describe('Lista separada por vÃ­rgula, ex: \'matutino,vespertino\'.'),
  "resolucaoSeedPr": zod.string().nullish().describe('NÃºmero da resoluÃ§Ã£o SEED-PR (ou equivalente estadual) que rege a escola.'),
  ...
  "telefoneContato": zod.string().nullish(),
  "emailCobranca": zod.string().nullish().describe('E-mail especÃ­fico usado pelo Asaas pra notificar a escola (boleto\/PIX). Cai pra emailContato quando vazio.'),
  "asaasCustomerId": zod.string().nullish(),
  "asaasSubscriptionId": zod.string().nullish(),
  "asaasStatusAssinatura": zod.union([zod.literal('em_dia'),zod.literal('pendente'),zod.literal('atrasada'),zod.literal('cancelada'),zod.literal(null)]).nullish().describe('Alimentado pelo webhook do Asaas a cada evento de cobranÃ§a.'),
